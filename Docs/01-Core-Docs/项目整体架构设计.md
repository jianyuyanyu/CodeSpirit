...
graph TB
    subgraph "表示层 (Presentation Layer)"
        ...
        TaskAPI[CodeSpirit.TaskApi]
        GradeAPI[CodeSpirit.GradeApi]
    ...
    subgraph "横切关注点 (Cross-Cutting Concerns)"
        ...
        LLM[CodeSpirit.LLM]
        Generator[CodeSpirit.Generator]
...
- **.NET 10 + Aspire**: 使用最新的.NET技术栈
- **Entity Framework Core**: 现代化的ORM数据访问
- **SignalR**: 实时通信支持
- **Redis**: 高性能缓存和分布式锁
- **RabbitMQ**: 异步消息处理
