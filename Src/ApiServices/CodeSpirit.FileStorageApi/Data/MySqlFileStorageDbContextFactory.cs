...
/// <summary>
/// MySQL 文件存储系统数据库上下文工厂
/// 用于设计时工具（如迁移生成）
/// </summary>
public class MySqlFileStorageDbContextFactory : IDesignTimeDbContextFactory<MySqlFileStorageDbContext>
{
    /// <summary>
    /// 创建数据库上下文
    /// </summary>
    /// <param name="args">命令行参数</param>
    /// <returns>MySQL文件存储系统数据库上下文</returns>
    public MySqlFileStorageDbContext CreateDbContext(string[] args)
    {
        ...
        var connectionString = configuration.GetConnectionString("DefaultConnection") ?? "Server=localhost;Port=53362;Database=filestorage-api;Uid=root;Pwd=Password123;CharSet=utf8mb4;";
        if (string.IsNullOrEmpty(connectionString))
        {
            throw new InvalidOperationException("未找到数据库连接字符串 'DefaultConnection'");
        }

        var optionsBuilder = new DbContextOptionsBuilder<MySqlFileStorageDbContext>();
        ...
        return new MySqlFileStorageDbContext(
