...
/// <summary>
/// SQL Server 文件存储系统数据库上下文工厂
/// 用于设计时工具（如迁移生成）
/// </summary>
public class SqlServerFileStorageDbContextFactory : IDesignTimeDbContextFactory<SqlServerFileStorageDbContext>
{
    /// <summary>
    /// 创建数据库上下文
    /// </summary>
    /// <param name="args">命令行参数</param>
    /// <returns>SQL Server文件存储系统数据库上下文</returns>
    public SqlServerFileStorageDbContext CreateDbContext(string[] args)
    {
        ...
        var connectionString = configuration.GetConnectionString("DefaultConnection") ?? "Server=(localdb)\\mssqllocaldb;Database=filestorage-api;Trusted_Connection=True;MultipleActiveResultSets=true;";
        if (string.IsNullOrEmpty(connectionString))
        {
            throw new InvalidOperationException("未找到数据库连接字符串 'DefaultConnection'");
        }

        var optionsBuilder = new DbContextOptionsBuilder<SqlServerFileStorageDbContext>();
        ...
        return new SqlServerFileStorageDbContext(
