...
/// 用于设计时工具（如迁移生成）
/// </summary>
public class MySqlSurveyDbContextFactory : IDesignTimeDbContextFactory<MySqlSurveyDbContext>
{
    /// <summary>
    /// 创建数据库上下文
    /// </summary>
    /// <param name="args">命令行参数</param>
    /// <returns>MySQL问卷系统数据库上下文</returns>
    public MySqlSurveyDbContext CreateDbContext(string[] args)
    {
        ...
        var connectionString = configuration.GetConnectionString("DefaultConnection") ?? "Server=localhost;Port=53362;Database=survey-api;Uid=root;Pwd=Password123;CharSet=utf8mb4;";
        if (string.IsNullOrEmpty(connectionString))
        {
            throw new InvalidOperationException("未找到数据库连接字符串 'DefaultConnection'");
        }

        var optionsBuilder = new DbContextOptionsBuilder<MySqlSurveyDbContext>();
        ...
        return new MySqlSurveyDbContext(
