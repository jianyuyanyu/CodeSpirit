...
/// <summary>
/// SQL Server 问卷系统数据库上下文工厂
/// 用于设计时工具（如迁移生成）
/// </summary>
public class SqlServerSurveyDbContextFactory : IDesignTimeDbContextFactory<SqlServerSurveyDbContext>
{
    /// <summary>
    /// 创建数据库上下文
    /// </summary>
    /// <param name="args">命令行参数</param>
    /// <returns>SQL Server问卷系统数据库上下文</returns>
    public SqlServerSurveyDbContext CreateDbContext(string[] args)
    {
        ...
        var connectionString = configuration.GetConnectionString("DefaultConnection") ?? "Server=(localdb)\\mssqllocaldb;Database=survey-api;Trusted_Connection=True;MultipleActiveResultSets=true;";
        if (string.IsNullOrEmpty(connectionString))
        {
            throw new InvalidOperationException("未找到数据库连接字符串 'DefaultConnection'");
        }

        var optionsBuilder = new DbContextOptionsBuilder<SqlServerSurveyDbContext>();
        ...
        return new SqlServerSurveyDbContext(
